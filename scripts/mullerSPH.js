...
var Vector = function (x, y) {
	this.v = new Float32Array([x === undefined ? 0 : x, y === undefined ? 0 : y]);
};
...
Vector.prototype = {
	...
	lerp: function (v, t, out) {
		...
		return out.set(
			this.v[0] + (v.v[0] - this.v[0]) * t,
			this.v[1] + (v.v[1] - this.v[1]) * t
		);
	},
...
Bucket.prototype.forEach = function (data, callback) {
	for (var i = this.count - 1; i >= 0; i--) callback(data, this.values[i]);
};
...
Grid.prototype.clear = function () {
	for (var y = 0; y < this.ndivsY; y++)
		for (var x = 0; x < this.ndivsX; x++) this.buckets[y][x].clear();
...
var SPHSimulation = function (
	...
	this.kernelRadius = Math.sqrt(
		(this.kernelParticles * area) / (this.numParticles * Math.PI)
	);
	...
	this.marching = new MarchingSquares(
		this.width,
		this.height,
		this.ndivsX,
		this.ndivsY,
	...
	for (var i = 0; i < this.numParticles; i++) {
		...
		var y =
			0.5 +
			0.125 * this.height +
			Math.floor(i / this.ndivsX) * this.gridSizeY * 0.25;
	...
	this.wPoly6GradScalar =
		-945 / (32 * Math.PI * Math.pow(this.kernelRadius, 9)); // Smoothing kernel constant
	...
	this.wViscosityLaplacianScalar =
		45 / (Math.PI * Math.pow(this.kernelRadius, 6)); // Smoothing kernel constant
	...
	this.threshold =
		Math.sqrt(this.material.restDensity / this.kernelParticles) / 7; // Surface normal threshold
...
SPHSimulation.prototype.updateDensities = function () {
	...
	for (var i = this.numParticles - 1; i >= 0; i--) {
		...
		if (pi.alive) {
			...
			// Determine boundaries
			var minX = Math.floor(
				(pi.position.v[0] - this.kernelRadius) / this.gridSizeX
			);
			var minY = Math.floor(
				(pi.position.v[1] - this.kernelRadius) / this.gridSizeY
			);
			var maxX = Math.floor(
				(pi.position.v[0] + this.kernelRadius) / this.gridSizeX
			);
	...
	for (var i = this.numParticles - 1; i >= 0; i--) {
		...
		if (pi.alive)
			pi.pressure =
				this.material.gasConstant * (pi.density - this.material.restDensity);
	}
};
					...
					for (var k = bucket.count - 1; k >= 0; k--) {
						...
						if (j > i) {
							...
							if (rSqr <= this.kernelRadiusSqr) {
								...
								// Compute viscosity
								var viscLap =
									this.material.viscosityMu *
									this.wViscosityLaplacian(r, this.vaux);
...
SPHSimulation.prototype.integrate = function (deltaTime) {
	for (var i = this.numParticles - 1; i >= 0; i--) {
		...
		if (pi.alive) {
			...
			// Restitution
			if (pi.restCount > 0)
				pi.position.addScaled(
					pi.restitution,
					this.material.restitution / pi.restCount
				);
...
SPHSimulation.prototype.draw = function () {
	...
	for (var i = this.numParticles - 1; i >= 0; i--) {
		...
		if (pi.alive) {
			this.context.fillStyle = HsvColor(
				180 - Math.min((pi.density / 5000) * 180, 180),
				100,
				100
			);
...
MarchingSquares.prototype = {
	interpolate: function (x1, y1, x2, y2, value1, value2, out) {
		if (Math.abs(this.threshold - value1) < this.epsilon) {
			...
			Math.abs(this.threshold - value2) < this.epsilon ||
			Math.abs(value1 - value2) < this.epsilon
		) {
			out.set(x2, y2);
		} else {
